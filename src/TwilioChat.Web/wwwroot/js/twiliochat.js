var twiliochat = (function () {
    ...
    var generalChannelUniqueName = 'general';
    var generalChannelName = 'General Channel';
    var messagesHistoryLimit = 50;
    ...
    function fetchAccessToken(username, handler) {
        $.get("/api/twilio/gettoken?identity=" + username)
    ...
    tc.joinGeneralChannel = function () {
        ...
        if (!tc.generalChannel) {
            ...
            tc.messagingClient.createChannel({
                uniqueName: generalChannelUniqueName,
                friendlyName: generalChannelName
    ...
    tc.loadMessages = function () {
        tc.currentChannel.getMessages(messagesHistoryLimit).then(function (messages) {
    ...
    function updateChannelUI(selectedChannel) {
        ...
        if (tc.currentChannelContainer === undefined && selectedChannel.uniqueName === generalChannelUniqueName) {
    ...
    function addChannel(channel) {
        if (channel.uniqueName === generalChannelUniqueName) {
            tc.generalChannel = channel;
        }
    ...
    tc.sortChannelsByName = function (channels) {
        return channels.sort(function (a, b) {
            if (a.friendlyName === generalChannelName) {
                return -1;
            }
            if (b.friendlyName === generalChannelName) {
                return 1;
            }
